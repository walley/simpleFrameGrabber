...
static void wait_for_img(int fd)
{
...
//  static const char Sig[]="*RDY*";
  ...
  do {
    ...
    */
    if (tempC == 'R' && signature == 0) {
      printf("Got R\n");
      signature++;
    } else if (tempC == 'D' && signature == 1) {
      printf("Got D\n");
      signature++;
    } else if (tempC == 'Y' && signature == 2) {
      printf("Got Y\n");
      signature++;
    } else {
      signature = 0;
...
int create_jpeg(int filenum, int fd, size_t sz, uint8_t *jpg, char *image, int  width, int height)
{
  FILE * outf;
  char * str;
  unsigned x, y = 0;
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
    ...
    memcpy(image+(y*width*3),buffer[0],width*3);
    ++y;
  }
...
int main(int argc, char**argv)
{
  ...
  printf("baudrate: %s %i %i \n", argv[4], baudRate, baudTab[i].baudRate);
  ...
  printf("width: %d  height: %d colspace: %d baudrate: %d proto: %d\n",width,height,colspace,baudRate,protocolVersion);
  //SDL_Surface *screen = NULL;
  char *image;
//  if(colspace==RGB565)
//    image = SDL_CreateRGBSurface(SDL_SWSURFACE,width,height,16,be16toh(0xF800),be16toh(0x07e0),be16toh(0x1F),0);
//  else
  image = (char *) malloc(307200); //SDL_CreateRGBSurface(SDL_SWSURFACE,width,height,24,0x0000ff,0x00ff00,0xff0000,0);
  //Start SDL
...
//  SDL_Event event;
  ...
  */
  ...
  while (exitLoop) {
    ...
    if (colspace == JPEG) {
      wait_for_img(fd);
      create_jpeg(filenum++, fd, sz, jpg, image, width, height);
    } else {
      for (y=0; y<height; ++y) {
        if (protocolVersion) {
          wait_for_img(fd);
